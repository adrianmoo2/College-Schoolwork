	...
	switch (sm1_state) {
		...
		case sm1_pressed:
			...
			else
			{
				...
				column_sel = 0xFE;
				column_val = 0x08;
				if (PORTB > 0xF7 || (PORTB == 0xF7 && PORTA > 0x08))
				{
					LCD_DisplayString(1, "You were " + ledTimeLate + " ms too late!");
				}
				else 
				{
					LCD_DisplayString (1, "You were " + ledTimeEarly + " ms too early!");
				}
			}
			break;
		case sm1_reset:
...
void lvl2_Tick() {
	...
	switch (lvl2_state) {
		...
		case lvl2_pressed:
			//buttonTemp = 0x01;
			if (PORTA == 0x40 && PORTB == 0xFD)
			{
				column_sel_index_lvl2 = 1;
				column_val_index_lvl2 = 4;
			}
			else
			{
				column_sel_index_lvl2 = 7;
...
int main(void)
{
	...
	LCD_DisplayString(1, "Hello Worldss");
